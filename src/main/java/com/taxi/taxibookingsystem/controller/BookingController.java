...
public class BookingController {
    ...
    public String view(HttpSession session, Model model) {
        ...
        if ("ADMIN".equals(role)) {
            ...
            List<String> userBookings = allBookings.stream()
                    .filter(b -> b.startsWith(currentUser + " ("))
            ...
            List<String> historyBookings = userBookings.stream().filter(b -> {
                ...
                return status.startsWith("COMPLETED") || status.startsWith("DECLINED");
            }).collect(Collectors.toList());

            model.addAttribute("completedBookings", historyBookings);
